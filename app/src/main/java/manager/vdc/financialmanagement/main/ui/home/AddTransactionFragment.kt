...
import manager.vdc.financialmanagement.databinding.FragmentAddTransactionBinding
...
import manager.vdc.financialmanagement.main.rdb.vm_data.*
...
class AddTransactionFragment : BaseFragment() {
    ...
    private fun initView() {
        ...
        if (typeInputScreen == 0) {
            ...
            if (!dataViewMode.checkEdtTransaction) {
                dataViewMode.transactionAddOrEdt =
                    dataViewMode.selectTransactionByTimeToDefault.let { transaction ->
    ...
    private fun createViewEdtMoney(typeInputScreen: Int) {
        if (typeInputScreen == 1) {
            ...
            binding.edtTypeAmountTransaction0.text =
                transaction.moneyAccountWithDetails?.country?.currencyCode
            val transactionAmount =
                transaction.transactionWithDetails?.transaction?.transactionAmount!!
        ...
        dataViewMode.moneyAccountWithDetailsSelect.observe(requireActivity()) {
            if (it.moneyAccount!!.idMoneyAccount != 0) {
                ...
                if (dataViewMode.checkInputScreenAddTransaction == 1) {
                    ...
                    dataViewMode.transactionAddOrEdt.transactionWithDetails!!.transaction!!.idMoneyAccount =
                        it.moneyAccount.idMoneyAccount
                }
            }
        }
    ...
    private fun createRcv() {
        ...
        dataViewMode.listCategoryByTypeLiveData.observe(requireActivity()) {
            ...
            if (dataViewMode.checkInputScreenAddTransaction == 0) {
                ...
                if (idCategory != 0) {
                    dataViewMode.transactionAddOrEdt.transactionWithDetails?.category?.idCategory =
                        idCategory
                    adapterIconCategory.updateSelect(idCategory!!)
                } else {
        ...
        adapterIconCategory.setClickItemSelect {
            if (it.idCategory <= 2) {
                ...
                if (dataViewMode.checkInputScreenAddTransaction == 0) {
                    ...
                    dataViewMode.transactionAddOrEdt.transactionWithDetails?.transaction?.idCategory =
                        it.idCategory
    ...
    private fun setTimeByEditTransaction(inputTimeInMillis: Long) {
        ...
        val todayStart =
            calendarToday.clone() as Calendar // Tạo một bản sao của đối tượng Calendar hiện tại
        ...
        val yesterdayStart =
            calendarYesterday.clone() as Calendar // Tạo một bản sao của đối tượng Calendar của ngày hôm qua
    ...
    private fun checkDataAddOrEditTransaction(typeClick: Int): Boolean {
        if (dataViewMode.country.idCountry == 0) {
            ...
            try {
                ...
                Toast.makeText(requireContext(),
                    requireContext().getString(R.string.you_entered_the_wrong_format),
                    Toast.LENGTH_SHORT)
                    .show()
            }
        ...
        if (dataViewMode.transaction.idMoneyAccount == null) {
            Toast.makeText(requireContext(),
                requireContext().getString(R.string.text_select_account),
                Toast.LENGTH_SHORT)
                .show()
            return false
        }
        ...
        if (dataViewMode.transaction.idCategory == 0) {
            Toast.makeText(requireContext(),
                requireContext().getString(R.string.you_have_not_selected_a_category),
                Toast.LENGTH_SHORT)
                .show()
            return false
        }
    ...
    private fun checkDataEditTransaction(): Boolean {
        ...
        val typeCategoryDefault =
            dataViewMode.selectTransactionByTimeToDefault.transactionWithDetails?.category?.type
        val typeCategoryNew =
            dataViewMode.transactionAddOrEdt.transactionWithDetails?.category?.type


        // số tiền mặc định
        val amountDefault =
            dataViewMode.selectTransactionByTimeToDefault.transactionWithDetails?.transaction?.transactionAmount

        // tài khoản money mặc định
        val moneyAccountDefault =
            dataViewMode.selectTransactionByTimeToDefault.moneyAccountWithDetails?.moneyAccount
        ...
        var checkNewType = 1
        checkNewType =
            if (typeCategoryDefault == CategoryType.EXPENSE && typeCategoryNew == CategoryType.INCOME) {
                1
            } else {
                -1
            }
